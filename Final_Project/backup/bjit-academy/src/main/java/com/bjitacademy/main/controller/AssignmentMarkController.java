...
public class AssignmentMarkController {
    ...
    public ResponseEntity<Optional<AssignmentMark>> getBatchById(@PathVariable Long assignmentMarkstId){
    ...
    public ResponseEntity<AssignmentMark> createAssignment(@RequestBody AssignmentMark assignmentMark) {
    	log.info("In Assignment Mark Controller class ");
		URI uri = URI.create(ServletUriComponentsBuilder.fromCurrentContextPath().path("/api/v1/assignmentMarks").toUriString());
	...
	@PreAuthorize("hasAnyRole('ROLE_ADMIN','ROLE_TRAINER','ROLE_TRAINEE')")
    public ResponseEntity<AssignmentMark> updateBatchDetails(@PathVariable Long assignmentMarkId, @RequestBody AssignmentMark updatedAssignmentMark) {
